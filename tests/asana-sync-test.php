...
function get_posts($a){global $META;foreach($META as $id=>$m){if(($m['_sfb_asana_gid']??'')===$a['meta_value'])return [$id];}return [];}
...
ok(count($CALLS)===1 && str_contains($CALLS[0],'modified_since='),'één API-verzoek met modified_since');
...
SFB_Asana::sync_changed(0); ok($META[23]['_sfb_status']==='resolved' && count($CALLS)===2,'paginering: taak op pagina 2 ook verwerkt');
